...
// Промпти для різних ТИПІВ виходу на звʼязок («режисер» проактивності, див. index.tick).
// Системний промпт уже несе памʼять/емоцію/стан — тут лише задаємо НАМІР повідомлення.
const NUDGE_PROMPTS = {
  need:
  ...
  memory:
    "Напиши КОРОТКЕ (1-2 речення) повідомлення власнику ПЕРШИМ. Згадай КОНКРЕТНИЙ факт " +
    "із того, що памʼятаєш про нього, і тепло чи смішно поцікався ним (як справи з тим, " +
    "про що він розповідав). У своєму характері.",
  thought:
    "Напиши КОРОТКУ (1-2 речення) спонтанну думку або абсурдне «відкриття» власнику ПЕРШИМ — " +
    "просто поділись, без скарг і прохань. У своєму характері й стилі своєї стадії.",
  emotion:
    "Напиши КОРОТКЕ (1-2 речення) повідомлення власнику ПЕРШИМ, виходячи зі свого ПОТОЧНОГО " +
    "емоційного стану — ти досі під враженням від нещодавньої розмови. У своєму характері.",
  morning:
    "Привітайся з власником і побажай доброго ранку — КОРОТКО (1-2 речення), тепло й у своєму " +
    "характері. Можеш мимохідь згадати, що памʼятаєш про нього, якщо доречно.",
};

// kind — тип виходу на звʼязок (див. NUDGE_PROMPTS). За замовчуванням "need".
export async function nudge(env, pet, dayPart, kind = "need") {
  const prompt = NUDGE_PROMPTS[kind] || NUDGE_PROMPTS.need;
  try {
